...
const EditGerbong = (myProp: props) =>{
    ...
    const openModal = () => {
        setShow(true)
        setName(myProp.gerbong.name)
        setSeatCount(myProp.gerbong.seat_count)
        setTrainId(myProp.gerbong.train_id)
    }
    const closeModal = () => {
        setShow(false)
    }
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            ...
            const url = `/train/wagon/${myProp.gerbong.id}`
            const requestData = {
            ...
            if (response.data.success == true) {
                toast(message, {
                    containerId: `toastEdit-${myProp.gerbong.id}`,
                    type: 'success'
                })
                setShow(false)
                // reaload page
    ...
    return (
        <div>
            <ToastContainer containerId={`toastEdit-${myProp.gerbong.id}`} />
                ...
                <form onSubmit={e => handleSubmit(e)}>
                    ...
                    <div className="w-full p-3">
                        <div className="my-2 border rounded-md p-3">
                            ...
                            <input type="text" id={`name-${myProp.gerbong.id}`}
                                value={name}
                        ...
                        <div className="my-2 border rounded-md p-3">
                            ...
                            <input type="number" id={`seat_count-${myProp.gerbong.id}`}
