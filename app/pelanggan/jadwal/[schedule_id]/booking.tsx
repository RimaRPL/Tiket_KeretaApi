...
const Booking = (myProp: Props) => {
    ...
    useEffect(() => {
        // ...untuk mengcopy array dari prop "wagons" ke state "wagons"
    ...
    const handleAddSeat = (seatBook: SeatBook) => {
        ...
        // mencari posisi index dari gerbong yang mempunyai "seat-number" dari yang dipilih oleh pengguna
    ...
    const handleSave = async () => {
        try {
            if(details.length == 0) {
                toast(`Kamu harus pilih kursi dlu`, {
    ...
    return (
        <div>
            <ToastContainer containerId={`toastBook`}/>
            {
                myProp.wagons.map((item, index) => (
                    <div key={`keyWagon-${index}`}
                        className="w-full my-2 p-3 rounded-md border shadow-md">
            ...
            className="w-full rounded-lg p-2 my-2 bg-green-600 hover:bg-green-500 text-white">
                Pesan Sekarang
            </button>
        </div>
    )
